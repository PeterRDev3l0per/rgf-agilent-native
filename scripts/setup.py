...
Guides users through environment checks, IDE selection, build validation, and onboarding.
"""
...
SUPPORTED_IDES = {
    "1": ("Cursor IDE", "cursor", ".cursor/rules/agilent.md"),
    "2": ("VS Code", "code", ".vscode/settings.json"),
    "3": ("Antigravity IDE (Gemini)", "antigravity", ".agents/rules/agilent.md"),
    "4": ("PyCharm / JetBrains", "pycharm", ".idea/agilent.xml"),
    "5": ("Neovim / Terminal AI", "nvim", ".nvim/agilent.lua"),
    "6": ("Ninguno / Standalone Web App", "none", None),
}
...
def select_ide_interactive(non_interactive: bool = False, default_choice: str = "1"):
    print(f"\n{COLOR_BOLD}📁 Selección de Integración con IDE de IA:{COLOR_RESET}")
    ...
    if non_interactive or not sys.stdin.isatty():
        ...
        try:
            choice = input(f"\n  Seleccioná tu IDE preferido [1-6] (Por defecto: 1): ").strip()
...
==================================================================={COLOR_RESET}
  ...
  {COLOR_BOLD}Escenario 1: Creación Vía Agente de IA desde tu IDE ({ide_name}){COLOR_RESET}
  1. Abrí la app ejecutando {COLOR_GREEN}agilent{COLOR_RESET} o visitando {COLOR_GREEN}http://127.0.0.1:8000/app/{COLOR_RESET}
  2. En tu agente de IA (Cursor/Antigravity/VSCode), decile:
...
def run_setup(non_interactive: bool = False, check_only: bool = False):
    ...
    print(f"{COLOR_BOLD}🔍 Verificación de Entorno & Dependencias:{COLOR_RESET}")
